...
//===================================== qmax calib =================================
GPUd() float GPUdEdx::qmaxCorrection(const GPUParam& param, int padRow, float cpad, float ctime, float ky, float kz, float rmsy0, float rmsz0, float effPad, float effDiff, int type)
{
  ...
  if(type) return GaussConvolutionGamma4(py,pz, pky,pkz,sy,sz,tau); // not used due to performance?
  ...
  // pkz = 0.01;
  // pky = 0.01;
  if(!type) return GaussConvolution(py, pz, pky, pkz, sy, sz);// * length;
  // return length;
}
...
GPUd() float GPUdEdx::GaussConvolution(float x0, float x1, float k0, float k1, float s0, float s1)
{
  ...
  ///
  /// TF1 f1("f1","AliTPCClusterParamNEW::GaussConvolution(x,0,1,0,0.1,0.1)",-2,2)
