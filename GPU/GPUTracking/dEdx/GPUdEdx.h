...
class GPUdEdx
{
 public:
  GPUd() void clear() {}
  GPUd() void fillCluster(float qtot, float qmax, int padRow, float trackSnp, float trackTgl, const GPUParam& param, const TPCdEdxCalibrationSplines* splines, float z, float py, float pz) {}
...
GPUdnii() void GPUdEdx::fillCluster(float qtot, float qmax, int padRow, float trackSnp, float trackTgl, const GPUParam& GPUrestrict() param, const TPCdEdxCalibrationSplines* splines, float z, float py, float pz)
{
  if (mCount >= MAX_NCL) {
    return;
  }
  ...
  const float qMaxCorr = splines->interpolateqMax(region, py, pz, z, angleZ, angleY);
  const float qTotCorr = splines->interpolateqTot(region, py, pz, z, angleZ, angleY);
  qmax /= qMaxCorr;
  qtot /= qTotCorr;
