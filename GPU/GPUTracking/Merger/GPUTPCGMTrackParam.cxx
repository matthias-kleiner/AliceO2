      ...
      if (retVal == 0) // track is updated
      {
        ...
        if (dEdxOut && iWay == nWays - 1 && clusters[ihit].leg == clusters[maxN - 1].leg) {
          ...
          if (merger->GetConstantMem()->ioPtrs.clustersNative == nullptr) {
            ...
            qtot = cl.qTot;
            qmax = cl.qMax;
            py = cl.getPad() - int(cl.getPad() + 0.5f);
            pz = cl.getTime() - int(cl.getTime() + 0.5f);
          }
          dEdx.fillCluster(qtot, qmax, clusters[ihit].row, mP[2], mP[3], param, merger->GetConstantMem()->calibObjects.dEdxSplines, zz, py, pz);
        }
