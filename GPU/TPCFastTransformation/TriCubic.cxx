...
DataT TriCubicInterpolatorGPU<DataT>::operator()(const DataT z, const DataT r, const DataT phi, const InterpolationType type) const
{
  if (type == InterpolationType::Sparse) {
    return interpolateSparse(z, r, phi);
  } else {
    const VectorGPU<DataT, FDim> coordinates{{z, r, phi}}; // vector holding the coordinates
...
template <typename DataT>
DataT TriCubicInterpolatorGPU<DataT>::operator()(const DataT z, const DataT r, const DataT phi, const size_t derz, const size_t derr, const size_t derphi) const
{
  const VectorGPU<DataT, FDim> coordinates{{z, r, phi}}; // vector holding the coordinates
...
template <typename DataT>
void TriCubicInterpolatorGPU<DataT>::initInterpolator(const unsigned int iz, const unsigned int ir, const unsigned int iphi) const
{
...
template <typename DataT>
DataT TriCubicInterpolatorGPU<DataT>::evalDerivative(const DataT dz, const DataT dr, const DataT dphi, const size_t derz, const size_t derr, const size_t derphi) const
{
  //TODO optimize this
  DataT ret{};
...
template <typename DataT>
DataT TriCubicInterpolatorGPU<DataT>::extrapolation(const DataT valk, const DataT valk1, const DataT valk2) const
{
...
template <typename DataT>
DataT TriCubicInterpolatorGPU<DataT>::linearExtrapolation(const DataT valk, const DataT valk1) const
{
...
template <typename DataT>
DataT TriCubicInterpolatorGPU<DataT>::parabolExtrapolation(const DataT valk, const DataT valk1, const DataT valk2) const
{
...
template <typename DataT>
void TriCubicInterpolatorGPU<DataT>::calcCoefficients(const unsigned int iz, const unsigned int ir, const unsigned int iphi) const
{
  DataT cVals[64]{};
  setValues(iz, ir, iphi, cVals);

  // needed for first derivative
  const VectorGPU<DataT, 24> vecDeriv1A{
  ...
  const VectorGPU<DataT, 24> vecDeriv1B{
  ...
  const VectorGPU<DataT, 24> vecDeriv2A{
  ...
  const VectorGPU<DataT, 24> vecDeriv2B{
  ...
  const VectorGPU<DataT, 24> vecDeriv2C{
  ...
  const VectorGPU<DataT, 24> vecDeriv2D{
  ...
  const VectorGPU<DataT, 8> vecDeriv3A{{cVals[42], cVals[43], cVals[46], cVals[47], cVals[58], cVals[59], cVals[62], cVals[63]}};
  const VectorGPU<DataT, 8> vecDeriv3B{{cVals[40], cVals[41], cVals[44], cVals[45], cVals[56], cVals[57], cVals[60], cVals[61]}};
  const VectorGPU<DataT, 8> vecDeriv3C{{cVals[34], cVals[35], cVals[38], cVals[39], cVals[50], cVals[51], cVals[54], cVals[55]}};
  const VectorGPU<DataT, 8> vecDeriv3D{{cVals[32], cVals[33], cVals[36], cVals[37], cVals[48], cVals[49], cVals[52], cVals[53]}};
  const VectorGPU<DataT, 8> vecDeriv3E{{cVals[10], cVals[11], cVals[14], cVals[15], cVals[26], cVals[27], cVals[30], cVals[31]}};
  const VectorGPU<DataT, 8> vecDeriv3F{{cVals[8], cVals[9], cVals[12], cVals[13], cVals[24], cVals[25], cVals[28], cVals[29]}};
  const VectorGPU<DataT, 8> vecDeriv3G{{cVals[2], cVals[3], cVals[6], cVals[7], cVals[18], cVals[19], cVals[22], cVals[23]}};
  const VectorGPU<DataT, 8> vecDeriv3H{{cVals[0], cVals[1], cVals[4], cVals[5], cVals[16], cVals[17], cVals[20], cVals[21]}};

  // factor for first derivative
  const DataT fac1{0.5};
  const VectorGPU<DataT, 24> vfac1{{fac1, fac1, fac1, fac1, fac1, fac1, fac1, fac1, fac1, fac1, fac1, fac1, fac1, fac1, fac1, fac1, fac1, fac1, fac1, fac1, fac1, fac1, fac1, fac1}};

  // factor for second derivative
  const DataT fac2{0.25};
  const VectorGPU<DataT, 24> vfac2{{fac2, fac2, fac2, fac2, fac2, fac2, fac2, fac2, fac2, fac2, fac2, fac2, fac2, fac2, fac2, fac2, fac2, fac2, fac2, fac2, fac2, fac2, fac2, fac2}};

  // factor for third derivative
  const DataT fac3{0.125};
  const VectorGPU<DataT, 8> vfac3{{fac3, fac3, fac3, fac3, fac3, fac3, fac3, fac3}};

  // compute the derivatives
  const VectorGPU<DataT, 24> vecDeriv1Res{vfac1 * (vecDeriv1A - vecDeriv1B)};
  const VectorGPU<DataT, 24> vecDeriv2Res{vfac2 * (vecDeriv2A - vecDeriv2B - vecDeriv2C + vecDeriv2D)};
  const VectorGPU<DataT, 8> vecDeriv3Res{vfac3 * (vecDeriv3A - vecDeriv3B - vecDeriv3C + vecDeriv3D - vecDeriv3E + vecDeriv3F + vecDeriv3G - vecDeriv3H)};

  const VectorGPU<DataT, 64> matrixPar{
...
template <typename DataT>
DataT TriCubicInterpolatorGPU<DataT>::interpolateDense(const VectorGPU<DataT, 3>& pos) const
{
  ...
  const VectorGPU<DataT, FDim> vals0{{1, 1, 1}};   // z^0, r^0, phi^0
  const VectorGPU<DataT, FDim> vals2{pos * pos};   // z^2, r^2, phi^2
  const VectorGPU<DataT, FDim> vals3{vals2 * pos}; // z^3, r^3, phi^3

  const DataT valX[4]{vals0[FZ], pos[FZ], vals2[FZ], vals3[FZ]};
  const VectorGPU<DataT, 64> vecValX{
  ...
  const VectorGPU<DataT, 64> vecValY{
  ...
  const VectorGPU<DataT, 64> vecValZ{
...
template <typename DataT>
DataT TriCubicInterpolatorGPU<DataT>::interpolateSparse(const DataT z, const DataT r, const DataT phi) const
{
  const VectorGPU<DataT, FDim> coordinates{{z, r, phi}}; // vector holding the coordinates
  ...
  const VectorGPU<DataT, FDim> vals0{posRel};
  const VectorGPU<DataT, FDim> vals1{vals0 * vals0};
  const VectorGPU<DataT, FDim> vals2{vals0 * vals1};

  const int nPoints = 4;
  const VectorGPU<DataT, nPoints> vecValX{{1, vals0[FZ], vals1[FZ], vals2[FZ]}};
  const VectorGPU<DataT, nPoints> vecValY{{1, vals0[FR], vals1[FR], vals2[FR]}};
  const VectorGPU<DataT, nPoints> vecValZ{{1, vals0[FPHI], vals1[FPHI], vals2[FPHI]}};
  ...
  const MatrixGPU<DataT, nPoints> matrixA{matrA};
  const VectorGPU<DataT, nPoints> vecValXMult{matrixA * vecValX};
  const VectorGPU<DataT, nPoints> vecValYMult{matrixA * vecValY};
  const VectorGPU<DataT, nPoints> vecValZMult{matrixA * vecValZ};

  DataT result{};
  int ind = 0;
  for (int slice = 0; slice < nPoints; ++slice) {
    const VectorGPU<DataT, nPoints> vecA{vecValZMult[slice] * vecValYMult};
    for (int row = 0; row < nPoints; ++row) {
      const VectorGPU<DataT, nPoints> vecD{{cVals[ind], cVals[++ind], cVals[++ind], cVals[++ind]}};
      ++ind;
...
template <typename DataT>
const VectorGPU<DataT, 3> TriCubicInterpolatorGPU<DataT>::processInp(const VectorGPU<DataT, 3>& coordinates, const bool sparse) const
{
  VectorGPU<DataT, FDim> posRel{(coordinates - mGridProperties.getGridMin()) * mGridProperties.getInvSpacing()}; // needed for the grid index
  posRel[FPHI] = mGridProperties.clampToGridCircularRel(posRel[FPHI], FPHI);
  const VectorGPU<DataT, FDim> posRelN{posRel};
  posRel[FZ] = mGridProperties.clampToGridRel(posRel[FZ], FZ);
  posRel[FR] = mGridProperties.clampToGridRel(posRel[FR], FR);

  const VectorGPU<DataT, FDim> index{floorVEC(posRel)};
...
template <typename DataT>
void TriCubicInterpolatorGPU<DataT>::getDataIndexCircularArray(const int index0, const int dim, int arr[]) const
{
...
template <typename DataT>
void TriCubicInterpolatorGPU<DataT>::getDataIndexNonCircularArray(const int index0, const int dim, int arr[]) const
{
...
template <typename DataT>
typename TriCubicInterpolatorGPU<DataT>::GridPos TriCubicInterpolatorGPU<DataT>::findPos(const int iz, const int ir, const int iphi) const
{
...
template <typename DataT>
bool TriCubicInterpolatorGPU<DataT>::findEdge(const int iz, const int ir, const int iphi, GridPos& posType) const
{
  const int iR = 2;
  if (iz == 0 && ir == 0) {
    if (iphi == 0) {
...
template <typename DataT>
bool TriCubicInterpolatorGPU<DataT>::findLine(const int iz, const int ir, const int iphi, GridPos& posType) const
{
  const int iR = 2;
  //check line
  if (ir == 0) {
...
template <typename DataT>
bool TriCubicInterpolatorGPU<DataT>::findSide(const int iz, const int ir, const int iphi, GridPos& posType) const
{
...
template <typename DataT>
bool TriCubicInterpolatorGPU<DataT>::isInInnerVolume(const int iz, const int ir, const int iphi, GridPos& posType) const
{
...
template <typename DataT>
bool TriCubicInterpolatorGPU<DataT>::isSideRight(const int ind, const int dim) const
{
...
template <typename DataT>
bool TriCubicInterpolatorGPU<DataT>::isSideLeft(const int ind) const
{
  if (ind == 0) {
    return true;
  }
...
template <typename DataT>
void TriCubicInterpolatorGPU<DataT>::setValues(const int iz, const int ir, const int iphi, DataT cVals[64]) const
{
...
template class o2::tpc::TriCubicInterpolatorGPU<double>;
template class o2::tpc::TriCubicInterpolatorGPU<float>;
