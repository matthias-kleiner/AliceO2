...
/// \class DataContainer3DGPU
/// The DataContainer3DGPU class represents a simple method to store values on a large 3-Dim grid with ROOT io functionality.

/// \tparam DataT the type of data which is used during the calculations
template <typename DataT = double>
struct DataContainer3DGPU {
  ...
  DataContainer3DGPU(unsigned short nZ, unsigned short nR, unsigned short nPhi) : mZVertices{nZ}, mRVertices{nR}, mPhiVertices{nPhi}, mData(nZ * nR * nPhi){};

  ///< default constructor for Root I/O
  DataContainer3DGPU() = default;
  ...
  inline static DataContainer3DGPU<DataT>* loadFromFile(TFile& inpf, const char* name = "data");
 ...
 private:
  ...
  ClassDefNV(DataContainer3DGPU, 1)
};
...
int DataContainer3DGPU<DataT>::writeToFile(TFile& outf, const char* name) const
{
  if (outf.IsZombie()) {
  ...
  DataContainer3DGPU<DataTOut> containerTmp(mZVertices, mRVertices, mPhiVertices);
  containerTmp.getData() = std::vector<DataTOut>(mData.begin(), mData.end());

  outf.WriteObjectAny(&containerTmp, DataContainer3DGPU<DataTOut>::Class(), name);
  return 0;
}
...
bool DataContainer3DGPU<DataT>::initFromFile(TFile& inpf, const char* name)
{
  LOGP(info, "Init...1");
  ...
  LOGP(info, "Init...get from file 2a");
  DataContainer3DGPU<DataTIn>* dataCont = (DataContainer3DGPU<DataTIn>*)(inpf.Get(name));
  LOGP(info, "Init...2");
...
template <typename DataT>
DataContainer3DGPU<DataT>* DataContainer3DGPU<DataT>::loadFromFile(TFile& inpf, const char* name)
{
  if (inpf.IsZombie()) {
    LOGP(error, "Failed to read from file {}", inpf.GetName());
    return nullptr;
  }
  DataContainer3DGPU<DataT>* dataCont{nullptr};

  dataCont = reinterpret_cast<DataContainer3DGPU<DataT>*>(inpf.GetObjectChecked(name, DataContainer3DGPU<DataT>::Class()));
  if (!dataCont) {
    LOGP(error, "Failed to load {} from {}", name, inpf.GetName());
    return nullptr;
  }
...
template <typename DataT>
void DataContainer3DGPU<DataT>::print() const
{
