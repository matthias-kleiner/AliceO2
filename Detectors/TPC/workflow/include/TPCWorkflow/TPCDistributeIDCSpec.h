...
{
 public:
  TPCDistributeIDCSpec(const std::vector<uint32_t>& crus, const unsigned int timeframes, const unsigned int outlanes, const unsigned int firstTF)
    : mCRUs{crus}, mTimeFrames{timeframes}, mOutLanes{outlanes}, mProcessedCRU{{std::vector<unsigned int>(timeframes), std::vector<unsigned int>(timeframes)}}, mTFStart{{firstTF, firstTF + timeframes}}, mTFEnd{{firstTF + timeframes - 1, mTFStart[1] + timeframes - 1}}
  {
  ...
  void run(o2::framework::ProcessingContext& pc) final
  {
    ...
    if (mProcessedCRU[currentBuffer][relTF] == 2 * o2::tpc::CRU::MaxCRU) {
 ...
 private:
  ...
  uint32_t getCurrentTF(o2::framework::ProcessingContext& pc) const { return o2::framework::DataRefUtils::getHeader<o2::header::DataHeader*>(pc.inputs().get("idcsgroup"))->tfCounter; }
  ...
  void sendOutput(o2::framework::ProcessingContext& pc, const unsigned int currentOutLane, const bool currentBuffer, const unsigned int relTF)
  {
    ...
    for (unsigned int iCRU = 0; iCRU < mCRUs.size(); ++iCRU) {
      pc.outputs().adoptContainer(Output{gDataOriginTPC, TPCDistributeIDCSpec::getDataDescriptionIDC(), header::DataHeader::SubSpecificationType{iCRU + currentOutLane * CRU::MaxCRU}}, std::move(mIDCs[currentBuffer][iCRU][relTF]));
      pc.outputs().adoptContainer(Output{gDataOriginTPC, TPCDistributeIDCSpec::getDataDescription1DIDC(), header::DataHeader::SubSpecificationType{iCRU}}, std::move(mOneDIDCs[currentBuffer][iCRU][relTF]));
    }
  }
};

DataProcessorSpec getTPCDistributeIDCSpec(const std::vector<uint32_t>& crus, const unsigned int timeframes, const unsigned int outlanes, const unsigned int firstTF)
{
  std::vector<InputSpec> inputSpecs{
    InputSpec{"idcsgroup", ConcreteDataTypeMatcher{gDataOriginTPC, TPCAverageGroupIDCDevice::getDataDescriptionIDCGroup()}, Lifetime::Timeframe},
    InputSpec{"1didc", ConcreteDataTypeMatcher{gDataOriginTPC, TPCAverageGroupIDCDevice::getDataDescription1DIDC()}, Lifetime::Timeframe}};
  ...
  return DataProcessorSpec{
    ...
    inputSpecs,
    outputSpecs,
    AlgorithmSpec{adaptFromTask<TPCDistributeIDCSpec>(crus, timeframes, outlanes, firstTF)}};
}
