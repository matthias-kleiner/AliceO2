...
void calculateDistortionsAnalytical(const int globalEFieldTypeAna = 1, const int globalDistTypeAna = 1, const int eFieldTypeAna = 1, const int usePoissonSolverAna = 1, const int nSteps = 1, const int simpsonIterations = 3, o2::tpc::Side side = o2::tpc::Side::C);

template <typename DataT, size_t Nz, size_t Nr, size_t Nphi>
void calculateDistortionsFromHist(const char* path, const char* histoName, const int globalEFieldType = 1, const int globalDistType = 1, const int nSteps = 1, const int simpsonIterations = 3);
...
void calcDistAna(const int gridType = 0, const int globalEFieldTypeAna = 1, const int globalDistTypeAna = 1, const int eFieldTypeAna = 1, const int usePoissonSolverAna = 1, const int nSteps = 1, const int simpsonIterations = 3, o2::tpc::Side side = o2::tpc::Side::C)
{
  if (gridType == 0) {
    calculateDistortionsAnalytical<double, 129, 129, 180>(globalEFieldTypeAna, globalDistTypeAna, eFieldTypeAna, usePoissonSolverAna, nSteps, simpsonIterations, side);
  } else if (gridType == 1) {
    calculateDistortionsAnalytical<double, 65, 65, 180>(globalEFieldTypeAna, globalDistTypeAna, eFieldTypeAna, usePoissonSolverAna, nSteps, simpsonIterations, side);
  } else if (gridType == 2) {
    calculateDistortionsAnalytical<double, 33, 33, 180>(globalEFieldTypeAna, globalDistTypeAna, eFieldTypeAna, usePoissonSolverAna, nSteps, simpsonIterations, side);
  } else if (gridType == 3) {
    calculateDistortionsAnalytical<double, 17, 17, 90>(globalEFieldTypeAna, globalDistTypeAna, eFieldTypeAna, usePoissonSolverAna, nSteps, simpsonIterations, side);
  }
}
...
/// \param nSteps number of which are used for calculation of distortions/corrections per z-bin
/// \param simpsonIterations number of iterations used in the simpson intergration
void calcDistFromHist(const char* path, const char* histoName, const int gridType = 0, const int globalEFieldType = 1, const int globalDistType = 1, const int nSteps = 1, const int simpsonIterations = 3)
{
  if (gridType == 0) {
    calculateDistortionsFromHist<double, 129, 129, 180>(path, histoName, globalEFieldType, globalDistType, nSteps, simpsonIterations);
  } else if (gridType == 1) {
    calculateDistortionsFromHist<double, 65, 65, 180>(path, histoName, globalEFieldType, globalDistType, nSteps, simpsonIterations);
  } else if (gridType == 2) {
    calculateDistortionsFromHist<double, 33, 33, 180>(path, histoName, globalEFieldType, globalDistType, nSteps, simpsonIterations);
  } else if (gridType == 3) {
    calculateDistortionsFromHist<double, 17, 17, 90>(path, histoName, globalEFieldType, globalDistType, nSteps, simpsonIterations);
  }
}

/// calculate distortions and corrections using an input histogram containing the density.
...
template <typename DataT, size_t Nz, size_t Nr, size_t Nphi>
void calculateDistortionsCorrections(o2::tpc::SpaceCharge<DataT, Nz, Nr, Nphi>& spaceChargeCalc, const o2::tpc::Side side, const int globalEFieldType, const int globalDistType)
{
  ...
  spaceChargeCalc.calcLocalDistortionsCorrections(dist, numEFields); // local distortion calculation
  stop = timer::now();
  time = stop - start;
  ...
  spaceChargeCalc.calcLocalDistortionsCorrections(corr, numEFields); // local correction calculation
  stop = timer::now();
  time = stop - start;
...
void calculateDistortionsAnalytical(const int globalEFieldTypeAna, const int globalDistTypeAna, const int eFieldTypeAna, const int usePoissonSolverAna, const int nSteps, const int simpsonIterations, o2::tpc::Side side)
{
...
/// \param simpsonIterations number of iterations used in the simpson intergration
template <typename DataT = double, size_t Nz = 17, size_t Nr = 17, size_t Nphi = 90>
void calculateDistortionsFromHist(const char* path, const char* histoName, const int globalEFieldType, const int globalDistType, const int nSteps, const int simpsonIterations)
{
  const auto integrationStrategy = o2::tpc::SpaceCharge<DataT, Nz, Nr, Nphi>::IntegrationStrategy::SimpsonIterative;
  o2::tpc::SpaceCharge<DataT, Nz, Nr, Nphi> spaceCharge3D;
  spaceCharge3D.setOmegaTauT1T2(0.32f, 1, 1);
  ...
  for (int iside = 0; iside < 2; ++iside) {
    ...
    calculateDistortionsCorrections(spaceCharge3D, side, globalEFieldType, globalDistType);
    // write to root file
    pcstream.GetFile()->cd();
    writeToTree(spaceCharge3D, pcstream, side);
  }
